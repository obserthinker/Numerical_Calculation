...
double* InitMatrix(int n_row,int n_column,double *matrix,int special,double InitValue);
...
double* DiagMatrix(int n_row,int n_column,double *matrix,int number);
...
double* InitVector(int n_row,int n_column,double *matrix);
...
Inputs: Three argument are required. The first is the number of row. It should be a positive int.
            ...
            special =0, all element are InitValue; special = 1, use pow() to make a matrix;
            special =2, assign the value from user.
...
double* InitMatrix(int n_row,int n_column,double *matrix,int special,double InitValue)
{
    if(special == 0){
        ...
        double value = InitValue;//define the matrix's value
...
Notes:
************************************************************/
double* InitVector(int n_row,int n_column,double *matrix,double InitValue)
{
    if(InitValue == 0){
        int i,j;//counting the row and column
        int n_column=1;
        ...
        *(B+0) = (double)(n_row);
        ...
        int i,j;//counting the row and column
        matrix = (double *)malloc((n_row*n_column)*sizeof(double));//allocate the space for the matrix
        double coun = InitValue;//initialize the matrix's value
        for(i = 0;i < n_row;i++){//for row
            for(j = 0;j < n_column;j++){//for column
